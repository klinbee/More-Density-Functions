...
public record ValueNoise(Interpolator interpolator, Optional<Integer> saltHolder,
    ...
    sealed interface Interpolator
        ...
        static Interpolator create(Type interpolation, boolean is2D,
                                   int sizeX, int sizeY, int sizeZ,
                                   RandomSampler randomSampler, int salt) {
            if (is2D) {
                return switch (interpolation) {
                    case NONE -> new None2D(interpolation, sizeX, sizeY, sizeZ, randomSampler, salt);
                    case LERP ->
                            new Lerp2D(interpolation, Interpolator::calcCellCoord, sizeX, sizeY, sizeZ, randomSampler, salt);
                    case SMOOTHSTEP ->
                            new Lerp2D(interpolation, Interpolator::calcCellCoordSmoothstep, sizeX, sizeY, sizeZ, randomSampler, salt);
                };
            }
            return switch (interpolation) {
                case NONE -> new None3D(interpolation, sizeX, sizeY, sizeZ, randomSampler, salt);
                case LERP ->
                        new Lerp3D(interpolation, Interpolator::calcCellCoord, sizeX, sizeY, sizeZ, randomSampler, salt);
                case SMOOTHSTEP ->
                        new Lerp3D(interpolation, Interpolator::calcCellCoordSmoothstep, sizeX, sizeY, sizeZ, randomSampler, salt);
            };
        }
        ...
        // 2D Raw Value Cells
        record None2D(Type interpolation,
                      int sizeX, int sizeY, int sizeZ,
        ...
        // 3D Raw Value Cells
        record None3D(Type interpolation,
                      int sizeX, int sizeY, int sizeZ,
                      RandomSampler randomSampler, int salt) implements Interpolator {
            @Override
            public double interpolate(int x, int y, int z) {
        ...
        record Lerp2D(Type interpolation, CellCalculator cellCalculator,
                      int sizeX, int sizeY, int sizeZ,
                      RandomSampler randomSampler, int salt) implements Interpolator {
            @Override
            public double interpolate(int x, int y, int z) {
        ...
        // Smoothstep optionally done through CellCalculator
        record Lerp3D(Type interpolation, CellCalculator cellCalculator,
                      int sizeX, int sizeY, int sizeZ,
                      RandomSampler randomSampler, int salt) implements Interpolator {
            @Override
            public double interpolate(int x, int y, int z) {
