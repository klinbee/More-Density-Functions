...
public record FloorModulo(DensityFunction dividend, DensityFunction divisor, Optional<Double> maxOutput, Optional<Double> minOutput, Optional<DensityFunction> errorDf) implements DensityFunction {

    private static final MapCodec<FloorModulo> DATA_CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(DensityFunction.HOLDER_HELPER_CODEC.fieldOf("base").forGetter(FloorModulo::dividend), DensityFunction.HOLDER_HELPER_CODEC.fieldOf("exponent").forGetter(FloorModulo::divisor), Codec.DOUBLE.optionalFieldOf("max_output").forGetter(FloorModulo::maxOutput), Codec.DOUBLE.optionalFieldOf("min_output").forGetter(FloorModulo::minOutput), DensityFunction.HOLDER_HELPER_CODEC.optionalFieldOf("error_output").forGetter(FloorModulo::errorDf)).apply(instance, FloorModulo::new));
    ...
    public double compute(@NotNull FunctionContext pContext) {
        ...
        double result = Math.floorMod(dividendValue, divisorValue);
    ...
    public @NotNull DensityFunction mapAll(DensityFunction.Visitor pVisitor) {
        return pVisitor.apply(new FloorModulo(this.dividend, this.divisor, this.maxOutput, this.minOutput, this.errorDf));
    }
